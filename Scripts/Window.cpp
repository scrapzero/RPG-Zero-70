...
void CEquipmentWindow::Draw()
{
	...
	if (equipmentKind<=2) {
		DrawFormatString(16, 395, BLACK, "%s   No.%d  îÑílÅF%dÅ@ëÆê´:%s", name.c_str(),KindNum, SellMoney, Element.c_str());
	}
	else {
		DrawFormatString(16, 395, BLACK, "%s     îÑílÅF%d", name.c_str(), SellMoney);
	}
